...
- **Markdown格式支持**：支持Markdown格式的消息内容，自动转换为HTML显示
- **字体大小调整**：支持动态调整聊天界面的字体大小
- **消息撤回功能**：支持撤回已发送的消息
...
- **API密钥保护**：请妥善保管API密钥，避免泄露
- **数据库密码保护**：数据库密码请使用强密码，并定期更换
- 建议定期备份对话历史和记忆数据
- 启用SSL验证以提高网络安全性
...
### 2025-12-17 更新
- **移除加密模块**：取消了API密钥和数据库凭证的加密功能，简化了代码结构
- **Markdown格式支持增强**：改进了Markdown转换逻辑，确保在没有安装markdown库时也能正常显示
- **消息撤回功能优化**：实现了更可靠的消息撤回机制，支持通过右键菜单直接撤回消息
- **快捷键修复**：修复了快捷键导入问题，确保Ctrl+Enter发送消息等快捷键正常工作
- **UI体验优化**：
  - 增强了右键菜单功能，支持复制和撤回消息
  - 优化了消息显示样式，确保所有消息从左向右显示
  - 改进了字体大小调整功能，支持动态更新聊天界面字体
- **性能优化**：
  - 优化了聊天显示刷新逻辑，使用批量HTML构建提升性能
  - 改进了消息处理机制，减少UI更新次数
- **错误处理增强**：
  - 修复了Markdown转换失败时的错误处理
  - 增强了快捷键导入的错误处理
  - 改进了消息撤回功能的错误处理
- **依赖管理**：添加了markdown库作为可选依赖，提升富文本显示能力
